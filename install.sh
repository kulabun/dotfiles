...
ask_confirmation() {
  echo "This script does a clean NixOS installation."
  echo "Hard drive will be formatted and all the data removed."
  while true; do
    read -p "[?] Do you want to continue? [Y/N] " confirm
      case $confirm in
        [Yy]* ) break;;
        [Nn]* ) echo "NixOS installation cancelled."; exit;;
...
run_nixossetup(){
  ...
  wget -q https://raw.githubusercontent.com/1000101/ni/master/configuration.nix -O /mnt/etc/nixos/configuration.nix
  if [ $ENCRYPTION ]; then
